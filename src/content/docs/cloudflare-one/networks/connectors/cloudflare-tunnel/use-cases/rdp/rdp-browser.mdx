...
12. <Render file="access/access-choose-idps" product="cloudflare-one" params={{ appType: "rdp" }}/>
...
Connection settings restrict data transfer between the user's local machine and the browser-based RDP session. You can control two types of data transfer:

- **Text controls** — Copy and paste text between the local machine and the remote session.
- **File controls** <Badge text="Beta" variant="caution" size="small" /> — Transfer files between the local machine and the remote session.

These controls are configured per policy, so you can grant different permissions to different groups of users.

### Default behavior

- **New policies**: Both text controls and file controls are denied by default. You must explicitly allow each action.
- **Existing applications**: Existing applications retain full text clipboard access for backward compatibility. File controls are denied unless explicitly enabled.
...
When a user attempts a restricted clipboard action, the clipboard content is replaced with a message informing them that the action is not allowed. When a user attempts a restricted file transfer, the transfer fails with an error message.
...
:::note
**PLACEHOLDER — LIMITS TO BE CONFIRMED:**
- **Maximum file size:** **`<MAX_FILE_SIZE>`**
- **Maximum files per upload:** **`<MAX_FILES_PER_UPLOAD>`**
:::

#### Upload files (local to remote)

To transfer files from your local machine to the remote Windows session, use one of the following methods:

- **Drag and drop**: Drag one or more files from your local machine directly onto the browser-based RDP window. The file lands on the active element of the remote desktop. For example, if you have a folder open in File Explorer, the file lands in that folder. If no folder is open, the file lands on the desktop.
- **Control panel**: Select the settings gear icon to open the control panel, then select the upload icon (third icon from the top). A file picker opens where you can select one or more files to upload. Files do not need to be the same type.
...
:::note
Local printing from the control panel is only available for PDF files.
:::

#### Limitations

- Only individual files are supported. Directory uploads are not available.
