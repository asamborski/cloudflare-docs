...
For SSH connections to [infrastructure applications](/cloudflare-one/access-controls/applications/non-http/infrastructure-apps/), Access also supports [MFA with PIV keys](#infrastructure-applications).
...
Infrastructure applications that use SSH support MFA with [YubiKey PIV keys](/cloudflare-one/access-controls/access-settings/independent-mfa/#enroll-a-piv-key-for-ssh). When MFA is required, the SSH proxy prompts the user for public key authentication using their enrolled PIV key before completing the connection. Users must [enroll their PIV key](/cloudflare-one/access-controls/access-settings/independent-mfa/#enroll-a-piv-key-for-ssh) through the App Launcher before they can connect.

You can configure MFA for SSH at the application level or at the policy level. Like other Access applications, policy-level settings override application-level settings.
...
Add the `mfa_settings` object to your infrastructure application:
...
--data '{
  "mfa_settings": {
    ...
    "mfa_bypass": false,
    "mfa_session_duration": 720,
    "mfa_allowed_authenticators": ["infrastructure_ssh"]
  }
}'
```
...
Add the `mfa_settings` object to the policy within your infrastructure application. The `mfa_settings` object uses the same fields as the [application-level configuration](#configure-mfa-for-an-infrastructure-application).
...
| Disabled | Required | (not set) | MFA required (application wins) |
| Disabled | Disabled | Required | MFA required (policy wins) |
| Required | (not set) | (not set) | MFA required (organization setting applies) |

Explicit settings at a lower level always override higher levels. If no explicit setting exists at a level, the next higher level applies.
