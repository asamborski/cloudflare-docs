...
Administrators can rename tools and prompts and edit their descriptions at the portal or server level without modifying the upstream MCP server. Aliases override the original tool and prompt metadata that MCP clients receive when they connect to the portal.
...
Alias names must follow the [MCP specification](https://modelcontextprotocol.io/specification/2025-11-25/server/tools) requirements: 1–64 characters, using only `A-Z`, `a-z`, `0-9`, `_`, `-`, `.`, and `/`. Names must be unique within a single MCP server.
...
4. In the servers table, select the **Tools authorized** or **Prompts authorized** entry for the server you want to configure. This value is displayed as a fraction (for example, `10/10`) indicating how many tools or prompts are enabled out of the total available.
...
:::note
If the upstream MCP server changes a tool or prompt name during a sync, the portal continues to use the administrator's alias. The alias takes precedence over any upstream changes.
:::
