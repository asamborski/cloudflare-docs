...
## 4. (Optional) Require MFA

You can require users to authenticate with a [YubiKey PIV key](/cloudflare-one/access-controls/access-settings/independent-mfa/#enroll-a-piv-key-for-ssh) before connecting to SSH targets. MFA can be configured at the application level or at the policy level, allowing you to enforce stricter requirements for sensitive usernames.
